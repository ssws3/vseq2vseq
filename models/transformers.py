...
class ConditionalTransformerModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if padding_frames > 0:
            # Pad the start of conditioning_hidden_states with zeros
            ...
            conditioning_attention_mask = torch.cat([torch.ones(batch_h * height_c * width_c, padding_frames), torch.zeros(batch_h * height_c * width_c, num_frames_c)], dim=1)

            conditioning_attention_mask = (1 - conditioning_attention_mask.to(conditioning_hidden_states.dtype)) * -10000.0
            conditioning_attention_mask = conditioning_attention_mask.unsqueeze(1).to(conditioning_hidden_states.device).half()
        else:
